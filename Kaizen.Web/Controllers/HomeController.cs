...
public class HomeController(ApplicationDbContext db, GoalProgressService progress, RegisterDailyAction registerDailyAction, UndoDailyAction undoDailyAction, ReorderDailyActions reorderDailyActions, ILogger<HomeController> logger) : Controller
{
    ...
    [HttpPost, ValidateAntiForgeryToken] public async Task<IActionResult> Ordenar(int[] ids, CancellationToken ct) { try { var result = await reorderDailyActions.ExecuteAsync(ids, DateOnly.FromDateTime(DateTime.Today), ct); if (!result.Success) return BadRequest(new { message = result.Message }); return Json(new { message = result.Message }); } catch (Exception ex) { logger.LogError(ex, "No se pudo guardar el orden de las acciones diarias."); return StatusCode(500, new { message = "No se pudo guardar el orden. Intentá nuevamente." }); } }
}
